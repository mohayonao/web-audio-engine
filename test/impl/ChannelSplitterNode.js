  ...
  describe("processing", () => {
    it("works", () => {
      ...
      [ 0, 1, 2, 3, 4, 5 ].forEach((ch) => {
        node2.getOutput(ch).getAudioBus().getMutableData()[0].set(np.random_sample(16));
      });
      node1.getOutput(0).getAudioBus().getMutableData()[0].set(noise1);
      node1.getOutput(0).getAudioBus().getMutableData()[1].set(noise2);
      node1.getOutput(0).getAudioBus().getMutableData()[2].set(noise1);
      node1.getOutput(0).getAudioBus().getMutableData()[3].set(noise2);

      context.process();

      const actual = [ 0, 1, 2, 3, 4, 5 ].map(ch => node2.getOutput(ch).getAudioBus().getChannelData()[0]);
      const isSilent = [ 0, 1, 2, 3, 4, 5 ].map(ch => node2.getOutput(ch).getAudioBus().isSilent());
